...
const ReportModal = ({ closeModal, branchName, appData, realityData, day, monthAndYear }: { closeModal: () => void, branchName: string, appData: AppData, realityData: RealityData, day: number, monthAndYear: string }) => {
    ...
    const discrepancyBetweenAppAndReality = discrepancyANumberOfGlasses.mGlass * 15000 + discrepancyANumberOfGlasses.lGlass * 15000 + discrepancyANumberOfGlasses.glass800 * 10000 - discrepancyMoney // Calculate the discrepancy between the number of glasses sold and the number of glasses sold on the force machine
    ...
    const downloadExcel = () => {
        ...
        const data = [
            ...
            { 'Mục': discrepancyANumberOfGlasses.lGlass != 0 ? 'Bấm ' + (discrepancyANumberOfGlasses.lGlass < 0 ? "DƯ" : "THIẾU") + ' ' + Math.abs(discrepancyANumberOfGlasses.lGlass) + ' ly L' : " ", 'Chênh lệch': discrepancyANumberOfGlasses.lGlass !== 0 ? Math.abs(discrepancyANumberOfGlasses.lGlass) * 15000 + ' VND' : " " },
            ...
            <div className='w-full sm:w-[700px] h-fit bg-white p-4 sm:p-6 border-[2px] border-dashed border-slate-500'>
                ...
                <div className='pt-5'>
                    ...
                    <div>
                        ...
                        {discrepancyANumberOfGlasses.lGlass !== 0 && <p>Bấm {discrepancyANumberOfGlasses.lGlass < 0 ? "DƯ" : "THIẾU"} {Math.abs(discrepancyANumberOfGlasses.lGlass)} ly L: <span className='font-bold text-lg text-red-500'>{formatNumber(Math.abs(discrepancyANumberOfGlasses.lGlass) * 15000)}</span> VND</p>}
                ...
                <div className='pt-5 pb-2 flex justify-end gap-x-4'>
                    ...
                    <button onClick={downloadExcel} className='px-6 py-2 bg-[#15B392] rounded-md text-white text-lg hover:opacity-80 transition-all'>Xuất file báo cáo</button>
