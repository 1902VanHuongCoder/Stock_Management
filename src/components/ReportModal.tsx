...
const ReportModal = ({ closeModal, branchName, appData, realityData, day, monthAndYear }: { closeModal: () => void, branchName: string, appData: AppData, realityData: RealityData, day: number, monthAndYear: string }) => {
    ...
    const downloadExcel = () => {
        ...
        const data = [
            ...
            { 'Mục': discrepancyANumberOfGlasses.lGlass != 0 ? 'Bấm ' + (discrepancyANumberOfGlasses.lGlass < 0 ? "DƯ" : "THIẾU") + ' ' + Math.abs(discrepancyANumberOfGlasses.lGlass) + ' ly L' : " ", 'Chênh lệch': discrepancyANumberOfGlasses.lGlass !== 0 ? Math.abs(discrepancyANumberOfGlasses.lGlass) * 20000 + ' VND' : " " },
